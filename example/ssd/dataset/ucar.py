...
class Ucar(Imdb):
    ...
    def __init__(self, image_set, devkit_path, shuffle=False, is_train=False):
        super(Ucar, self).__init__('Ucar'+ '_' + image_set)
        self.image_set = image_set
        self.devkit_path = devkit_path
        self.data_path = os.path.join(devkit_path, 'Ucar')
        self.extension = '.jpg'
        self.is_train = is_train
        self.year = 2012

        self.classes = ['1', '2', '3', '20']
    ...
    def evaluate_detections(self, detections):
        ...
        if not os.path.exists(result_dir):
            os.mkdir(result_dir)
        year_folder = os.path.join(self.devkit_path, 'results', 'Ucar')
        if not os.path.exists(year_folder):
            os.mkdir(year_folder)
        res_file_folder = os.path.join(self.devkit_path, 'results', 'Ucar', 'Main')
    ...
    def write_pascal_results(self, all_boxes):
        ...
        for cls_ind, cls in enumerate(self.classes):
            print('Writing {} Ucar results file'.format(cls))
